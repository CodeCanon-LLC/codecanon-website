...
| `data` | `WaraqData \| null` | — | Controlled canvas state. Pass `null` while data is loading to defer history initialisation; pass `undefined` (omit the prop) only for uncontrolled usage. |
| `onDataChange` | `(data: D) => void` | — | Called whenever layers, frame size, or background color change |
| `initialLayers` | `Layer[]` | `[]` | Uncontrolled initial layers (ignored in controlled mode) |
...
```tsx
<WaraqPanel position="right-start" collapsible>
  <WaraqPane>…</WaraqPane>
</WaraqPanel>
```
...
| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `position` | `PanelPosition` | `"left-start"` | Where to place the panel |
...
`PanelPosition` values: `"left-start"` | `"top"` | `"right-start"` | `"left-end"` | `"bottom"` | `"right-end"`
