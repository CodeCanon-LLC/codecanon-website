...
import { PriceTagDesignerDemo } from "@/apps/price-tag/components/price-tag-designer";
import { PriceTagDemo } from "@/apps/price-tag";
import { getWaraqPriceTagLink, getWaraqPriceTagTemplateLink } from "@/lib/links";

<Browser link={getWaraqPriceTagTemplateLink('price-tag-1')} className="not-prose">
  <PriceTagDesignerDemo />
</Browser>
...
The input layer renders an editable `<input>` or `<textarea>` directly on the canvas. It uses the `Component` form so it can call `useWaraqDocument` to push value changes back.
...
`useWaraqDocument()` gives access to `updateLayer`, `addLayer`, and the layer array. It can only be called from a component rendered inside `<Waraq>`.
