...
```tsx
<PresetPicker presets={presets} card={SomeCard} {...buttonProps} />
```
...
```tsx
<PresetDropdownPicker presets={presets} {...buttonProps} />
```
...
|---|---|---|---|
| `card` | `React.ComponentType<PresetPreviewCardProps>` | `PresetPreviewCard` | Custom preview card component. Must accept the same props as [PresetPreviewCard](#presetpreviewcard). |
