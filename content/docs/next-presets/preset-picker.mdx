...
| `previewCard` | `typeof PresetPreviewCard` | `PresetPreviewCard` | Custom preview card component. Must accept the same props. |
...
Pass a custom component to `PresetPickerContent` via the `previewCard` prop. It receives the same props as `PresetPreviewCard`:
