...
Then register it in the picker. **Prefer the Vite plugin** — it wires up the JS bundle automatically so no `PresetProvider` prop changes are needed:
...
export default defineConfig({
  plugins: [
    nextPresetsPlugin({
      ...
      // optionally limit built-ins too:
...
The picker and `usePreset().presets` will include `My Brand` automatically. No changes to `PresetProvider`.

### Option B — PresetProvider prop
...
const MY_PRESETS = [
  ...
  ...filterPresets(["nuteral", "claude"]), // or omit to include all built-ins
];
...
Either way, activate the preset with `setPreset`:
